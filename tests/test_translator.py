...
from translatex.translator import (
...
@pytest.mark.xfail(service_class=TEST_SERVICE_CLASSES[0],
                   reason="Google API key expired")
@pytest.mark.api
@pytest.mark.parametrize("service_class", TEST_SERVICE_CLASSES)
def test_translate_small(small_trans, service_class):
    small_trans.translate(
        service=service_class(), source_lang="en", destination_lang="fr"
    )
...
"""
    ...
    assert (
        TRANSLATION_SERVICE_CLASSES["Test service"]().translate("foo")
        == "Always yes"
    )
    assert (
        TRANSLATION_SERVICE_CLASSES["Test service 2"]().translate("foo")
        == "Always no"
    )
