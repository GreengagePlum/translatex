...
if the `infile` or `outfile` positional arguments are not provided, the program reads from `stdin` or writes to `stdout` respectively.
It also writes warnings about missing or altered indicators
...
You can provide your own translation service by creating a python file that provides a custom translation service class that derives from {class}`~translatex.translator.TranslationService` or one of its children and implements the {meth}`~translatex.translator.TranslationService.translate` method.
